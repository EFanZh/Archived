...
fn create_brush(render_target: &mut ID2D1HwndRenderTarget, color: &ColorF) -> ComPtr<ID2D1SolidColorBrush>
{
    let mut result = ComPtr::<ID2D1SolidColorBrush>::new();

    unsafe {
        let brush_result =
            render_target.CreateSolidColorBrush(&color.0, &BrushProperties::default().0, result.raw_addr());
...
impl Resource
{
    pub fn new(render_target: &mut ID2D1HwndRenderTarget, configuration: &Configuration) -> Resource
    {
        ...
        return Resource {
            ...
            tail_brushes: (0..TAIL_SHADES).map(|i| create_brush(render_target, &get_tail_brush_color(i))).collect()
        };
    }
