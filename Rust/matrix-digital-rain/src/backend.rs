...
impl Configuration {
    fn new() -> Configuration {
        return Configuration {
            ...
            character_candidates: "!\"#$%&()*+-/0123456789:;<=>?@".chars().collect(),
        };
    }
}
...
impl State {
    ...
    fn get_random_character(&mut self) -> char {
        return *self.random_number_generator
            .choose(self.configuration.character_candidates.as_slice())
            .unwrap();
    }

    fn get_time_to_birth(&mut self) -> f64 {
        return ((1.0 / (1.0 - self.random_number_generator.next_f64())) *
                self.configuration.lambda_generate)
                .log2();
    }

    fn generate_speed_value(&mut self) -> f64 {
        return self.random_number_generator
            .gen_range(self.configuration.minimal_speed,
                       self.configuration.maximal_speed);
    }
    ...
    fn update_column(&mut self,
        ...
        while raindrop_birth_time <= time_elapsed {
            ...
            let size = self.random_number_generator
                .gen_range(self.configuration.minimal_raindrop_size,
...
impl Backend {
    ...
    pub fn get_view(&mut self, columns: usize, rows: usize, time_ellapsed: f64) -> &[Vec<Raindrop>] {
        ...
        let mutation_probability =
            1.0 - 2.0f64.powf(-time_ellapsed / self.state.configuration.lambda_mutation);

        for rain_column in &mut self.rain_columns {
            self.state
                .update_column(rain_column, time_ellapsed, mutation_probability);
        }
